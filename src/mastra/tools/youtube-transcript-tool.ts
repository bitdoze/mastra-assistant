...
// Fetches captions for a YouTube video. Each transcript line is prefixed with
// a [MM:SS] timestamp. Use fetch-youtube-metadata first to get the title and
// context. Failures (no captions, unavailable, private) surface in `error`
// without throwing.
export const youtubeTranscript = createTool({
  ...
  execute: async (input) => {
    ...
    try {
      ...
      // YouTube rejects server-side innertube/timedtext requests from datacenter
      // IPs (anti-bot enforcement, PO-token requirement). This surfaces as a
      // "Bad Request" / "Failed to fetch player data" error. It is not a code
      // bug — use fetch-youtube-metadata (which still works) for context, or
      // route via a residential proxy / browser when available.
      if (
        errorMessage.includes("Bad Request") ||
        errorMessage.includes("Failed to fetch player data")
      ) {
        return {
          success: false,
          videoId,
          error:
            "YouTube blocked the transcript request (anti-bot). Transcripts are unavailable from this server's IP. Use fetch-youtube-metadata for video context, or fetch via a residential proxy or a real browser.",
        };
      }
